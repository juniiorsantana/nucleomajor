-- SOMENTE POSTGRES DESCARTAVEL: reproduz o corpo devolvido pelo usuario.
do $seed$
declare
  target oid := 'public.nucleo_customer_agent_handoff(text,text,text,text,text)'::regprocedure;
  original text;
  copied text := $copied$
  declare
    ...
    source_slug text;
    normalized_phone text := regexp_replace(coalesce(requester_phone, ''),
    '[^0-9]', '', 'g');
    max_handoffs constant integer := 3;
  begin
    if robot_org is null then raise exception 'active robot credential
    required'; end if;

    select * into context_row from public.conversation_intelligence_contexts
    context
    where context.organization_id = robot_org and context.channel =
    'whatsapp'
      and context.conversation_key_hash =
      nucleo_customer_agent_handoff.conversation_key_hash
      and context.state in ('active', 'handed_off') and context.audience =
      'customer'
    order by case when context.state = 'handed_off' then 0 else 1 end
    limit 1 for update;
    if not found then raise exception 'customer intelligence context
    required'; end if;
    ...
    if not target_agent.active then raise exception 'target agent inactive';
    end if;
    ...
    if handoff_reason is null or handoff_reason not in ('commercial_intent',
    'specialist_required', 'scope_mismatch') then
    ...
    select profile.slug into source_slug from public.assistant_profiles
    profile
    where profile.id = context_row.assistant_profile_id and
    profile.organization_id = robot_org;
    ...
    where context_id = context_row.id and organization_id = robot_org and
    status = 'active';
    ...
    insert into public.intelligence_audit_log (
      ...
      robot_org, auth.uid(), 'conversation', context_row.id,
      'agent_handoff',
      jsonb_build_object('sourceAgentSlug', source_slug, 'targetAgentSlug',
      target_agent.slug,
        'reason', handoff_reason, 'handoffCount',
        context_row.agent_handoff_count + 1)
    );
    return jsonb_build_object(
      ...
      'reason', handoff_reason, 'handoffCount',
      context_row.agent_handoff_count + 1,
      'message', 'Transferência confirmada; o agente destino atenderá no
      próximo turno.'
    );
  end;
  $copied$;
begin
  if md5(copied) <> '92928c87614ffd2603e9ecea9e376e5f' then
    raise exception 'incorrect fixture body';
  end if;
  select prosrc into original from pg_proc where oid=target;
  execute replace(pg_get_functiondef(target),original,copied);
end;
$seed$;

...
async function instalacaoAtual() {
  const dados = await webArea.get(CHAVE_INSTALACAO);
  ...
  await webArea.set({ [CHAVE_INSTALACAO]: id });
  return id;
}

async function todasAsCredenciais() {
  const dados = await webArea.get(CHAVE_CREDENCIAIS);
...
async function guardarCredencial({ organizationId, connectionId, installationId, token }) {
  ...
  await webArea.set({ [CHAVE_CREDENCIAIS]: guardadas });
}
...
async function esquecerCredenciais(predicado) {
  ...
  await webArea.set({ [CHAVE_CREDENCIAIS]: restantes });
}
...
export function criarOperacoesGateway() {
  ...
  let organizacaoRealtime = "";
  let temporizadorRealtime = null;

  const emitirMudancaRealtime = (organizationId, topic) => {
    if (typeof window === "undefined") return;
    clearTimeout(temporizadorRealtime);
    temporizadorRealtime = setTimeout(() => {
      window.dispatchEvent(new CustomEvent("emyleads:connections-changed", {
        detail: { organizationId, topic },
      }));
    }, 80);
  };

  const ativarRealtime = async (organizationId) => {
    const organizacao = exigirOrganizacao(organizationId);
    if (canalRealtime && organizacaoRealtime === organizacao) {
      return { ativo: true, organizationId: organizacao };
    }

    const supabase = obterSupabaseWeb();
    if (canalRealtime) await supabase.removeChannel(canalRealtime);
    organizacaoRealtime = organizacao;
    canalRealtime = supabase
      .channel(`portal-connections:${organizacao}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
          ...
          filter: `organization_id=eq.${organizacao}`,
        },
        ({ new: evento }) => emitirMudancaRealtime(organizacao, evento?.topic || "connections"),
      )
      .subscribe();
    return { ativo: true, organizationId: organizacao };
  };
